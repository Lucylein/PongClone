...
var render = function () {
	...
	if (player.doesntWantToPlay) {
		context.font = '48px serif';
		context.fillText('Press Q or A to play', 10, 50);
	}
...
Player.prototype.update = function () {
	for (var key in keysDown) {
		if (Number(key) == 81 || Number(key) == 65) {
			this.doesntWantToPlay = false;
		}
	}

	if (this.doesntWantToPlay) {
		this.paddle.move();
	} else {
		for (var key in keysDown) {
			if (Number(key) == 81) {
				this.paddle.move(0, -8);
			} else if (Number(key) == 65) {
				this.paddle.move(0, 8);
			} else {
